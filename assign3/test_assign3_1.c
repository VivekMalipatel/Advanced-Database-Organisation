...
//Professors code has been updated to fix the memory leaks
...
void activatePkeyTag() {
  printf("Pkey Tag Activated \n");
  globalPkeyTag = 1;
}
// Function that modifies the global variable
void deactivatePkeyTag() {
  printf("Pkey Tag DeActivated \n");
  globalPkeyTag = 0;
}
...
testSchemaTooBig(void)
{
  ...
  TEST_CHECK(initRecordManager(NULL));
  RC check=createTable("test_table_r",schema);
  ASSERT_ERROR(createTable("test_table_r",schema), "expected error 206");
  TEST_CHECK(shutdownRecordManager());
...
testRecordTooBig(void)
{
  ...
  TEST_CHECK(initRecordManager(NULL));
  ASSERT_ERROR(createTable("test_table_r",schema), "expected error code 207");
...
testPrimaryKey(void)
{
  ...
  //Here the pkey of first two records is 1
RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
  TestRecord inserts[] = { 
    {1, "aaaa", 3}, 
    {1, "bbbb", 2},
...
//Activate Primary key Tag
  ...
  RID *rids;
  Schema *schema;
  testName = "test creating a table to check where duplicate primary key cannot be inserted";
  schema = testSchema();
  rids = (RID *) malloc(sizeof(RID) * numInserts);
  ...
  for(i = 0; i < numInserts; i++)
    {
      r = fromTestRecord(schema, inserts[i%10]);
      char str2[5];
      sprintf(str2,"%d",i%10);
      char str1[100]="\n There is a primary key violation with record having pkey-->";
      strcat(str1,str2);
      int code;
      if (code=insertRecord(table,r)!=RC_OK)
      {
         ASSERT_ERROR(code,str1);
