...
void FIFO(BM_BufferPool *const bm, Frame *newPageFrame)
{
	...
	int i, frontIndex;
	frontIndex = readCnt % maxBufferSize;

	// Interating through all the page frames in the buffer pool
	for(i = 0; i < maxBufferSize; i++)
	{
		if(pageFrame[frontIndex].fCnt == 0)
		{
			// If page in memory has been modified (dirtyBit = 1), then write page to disk
			if(pageFrame[frontIndex].dirty == 1)
			{
				SM_FileHandle fh;
				openPageFile(bm->pageFile, &fh);
				writeBlock(pageFrame[frontIndex].page_num, &fh, pageFrame[frontIndex].page_data);
			...
			pageFrame[frontIndex].page_data = newPageFrame->page_data;
			pageFrame[frontIndex].page_num = newPageFrame->page_num;
			pageFrame[frontIndex].dirty = newPageFrame->dirty;
			pageFrame[frontIndex].fCnt = newPageFrame->fCnt;
			break;
		}
		else
		{
